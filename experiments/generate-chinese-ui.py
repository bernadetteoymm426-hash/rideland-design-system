...
# 阿里云百炼 API 配置
API_KEY = 'sk-sp-af34df8f2ea14f398a5a01b6087be65f'
API_URL = 'https://dashscope.aliyuncs.com/api/v1/services/aigc/text2image/image-synthesis'

# 输出目录
OUTPUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'generated-ui-images')
...
def generate_image(prompt, output_path):
    ...
    # 轮询获取结果
    return poll_task_result(task_id, output_path)

def poll_task_result(task_id, output_path, max_attempts=60):
    """轮询异步任务结果"""
    ...
    for i in range(max_attempts):
        ...
        phase = task_data.get('output', {}).get('phase', '')
        
        print(f'  轮询 {i + 1}/{max_attempts} - 状态：{phase}')
...
def main():
    ...
    # 逐个生成
    for i, ui in enumerate(UI_PROMPTS):
        output_path = os.path.join(OUTPUT_DIR, f'{ui["name"]}.png')
        
        try:
            generate_image(ui['prompt'], output_path)
            results.append({'name': ui['name'], 'path': output_path, 'status': 'success'})
        except Exception as e:
            errors.append({'name': ui['name'], 'error': str(e)})
            results.append({'name': ui['name'], 'status': 'failed', 'error': str(e)})
        
        # 每个生成之间等待一下，避免 API 限流
        if i < len(UI_PROMPTS) - 1:
            print('等待 3 秒后继续...\n')
            time.sleep(3)
    ...
    print(f'成功：{success_count}/{len(UI_PROMPTS)}')
    print(f'失败：{failed_count}/{len(UI_PROMPTS)}\n')
